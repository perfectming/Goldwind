...
let Component = React.createClass({
    ...
    render() {
            let {skinStyle,alertText,devtype,boolywbb=false,showtree,playjq,firstname,select_list,devurls='WindTurbine',searchnum,stateCheckData} = this.props;
            let treetype=[];//设备类型
            let one=[]; //一级菜单
            let two=[]; //二级菜单
            let three=[]; //三级菜单
            ...
            //表格数据处理
            if(stateCheckData!==undefined && stateCheckData.rectime!==undefined){
                let arrString0,arrString1,arrString2,arrString3,arrString4,arrString5;
                arrString0=stateCheckData.blooeydescr.Values.split(',');//设备状态
                arrString1=stateCheckData.code.Values.split(',');//状态代码
                arrString2=stateCheckData.rectime.Values.split(',');//状态时间
                arrString3=stateCheckData.datelength.Values.split(',');//时长描述
                arrString4=stateCheckData.timelength.Values.split(',');//状态时长
                for (var i=0 ; i< arrString0.length ; i++){
                    stateArr.push(arrString0[i]);
                }
                for (var i=0 ; i< arrString1.length ; i++){
                    stateCode.push(arrString1[i]/1);
                }
                for (var i=0 ; i< arrString2.length ; i++){
                    numberArr.push(new Date(arrString2[i]/1).toLocaleDateString()+' '+new Date(arrString2[i]/1).toLocaleTimeString());
                }
                for (var i=0 ; i< arrString3.length ; i++){
                    timeDescribe.push(arrString3[i]);
                }
                for (var i=0 ; i< arrString4.length ; i++){
                    timeArr.push(arrString4[i]);
                }
            }else{
                stateCheckData=undefined;
            }
            if(boolywbb){
                //获取设备类型数组
                    ...
                    <div className={skinStyle==1? styles.faultBoxBlue:skinStyle==2? styles.faultBoxWhite:styles.faultBox}>
                        ...
                        <div className={styles.righttable}>
                            <div className={styles.columnbox} id='colum'>
                                {stateCheckData!==undefined && <LineChart title={'设备状态查询用例规划'} month={numberArr} data={stateCode} unit1={'状态代码'}></LineChart>}
                            </div>
                                    ...
                                    <tbody>
                                        <tr className={styles.thState}><th>序号</th><th>状态时间</th><th>状态代码</th><th>设备状态</th><th>状态时长(s)</th><th>时长描述(h)</th></tr>
                                        {
                                            stateArr.map((value,key)=>{
                                                return(
                                                    <tr key={key} className={styles.thState}><th>{key+1}</th><th>{numberArr[key]}</th><th>{stateCode[key]}</th><th>{stateArr[key]}</th><th>{timeArr[key]}</th><th>{timeDescribe[key]}</th></tr>
                                                )
                                            })
                                        }
                                    </tbody>
...
const mapStateToProps = (state) => {
    return {
        ...
        stateCheckData:state.objs.stateCheckData,
    }
};
    ...
    return {
        ...
        searchnum:()=>{
            ...
            $.ajax({    
                ...
                data:'functionname=GetStatusInfo&wtid='+all+'&starttime='+startTime+'&endtime='+endTime+'&BTZ=480&crossDomain=true&zip=false',    
                dataType:"jsonp",    
                jsonp:"callback",    
                jsonpCallback:"testCall",    
                timeout:5000,       
                success:function(json,textStatus){ 
                    ...
                    dispatch(actions.setObjs('stateCheckData',json));
                },    
                error:function(XMLHttpRequest,textStatus,errorThrown){    
                    dispatch(actions.setVars('alertBool', false));
                    dispatch(actions.setVars('alertText', '请求超时！'));  
